const VideoTitle = ({ title, desc }) => {
  return (
    <div className="mx-4 w-1/2   md:mx-20 md:w-1/3 absolute top-1/3 flex flex-col gap-4">
      <div className="hidden md:block font-bold md:text-3xl text-white">{title}</div>
      <div className="hidden md:block text-white">{desc}</div>
      <div className="flex gap-4">
        ...
        <button className="hidden md:flex bg-[#6D6D6E] px-4 py-3 rounded-sm md:w-2/5 text-white font-bold justify-center gap-2 hover:bg-opacity-80">
          <img
            width="24"
            height="24"
